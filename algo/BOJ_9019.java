...
public class BOJ_9019 {
	...
	public static void bfs(String a, String b){
		...
		Queue<Answer> que = new LinkedList<Answer>();
		que.add(new Answer(a,""));
		...
		while(!que.isEmpty()){
			...
			for(int i=0;i<size;i++){
				...
				int twice = (t*2) % 10000;
				if(Integer.parseInt(b)==twice){
					clist += ans.c + "D";
					return ;
				}
				if(!check[twice]){
					que.add(new Answer(twice+"", ans.c+"D"));
					check[twice] = true;
				}
			
				int minus = t-1;
				if(t==0) minus = 9999;
				if(Integer.parseInt(b)==minus){
					clist += ans.c + "S";
					return ;
				}
				if(!check[minus]){
					que.add(new Answer(minus+"", ans.c+"S"));
					check[minus] = true;
				}

				int l = (t % 1000) * 10 + t/1000;
				
				if(Integer.parseInt(b)==l){
					clist += ans.c + "L";
					return ;
				}
				if(!check[l]){
					que.add(new Answer(l+"", ans.c+"L"));
					check[l] = true;
				}
				...
				if(Integer.parseInt(b)==r){
					clist += ans.c + "R";
					return ;
				}
				if(!check[r]){
					que.add(new Answer(r+"", ans.c+"R"));
					check[r] = true;
				}
